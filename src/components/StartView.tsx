  ...
  const handleSaveSettings = async () => {
    try {
      await apiCall('/api/me/settings', {
        method: 'POST',
        body: JSON.stringify({ 
          hourly_rate_pln: parseFloat(String(rate)) || 0, 
          tax_percent: parseFloat(String(tax)) || 0 
        })
      });
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div className="space-y-1.5">
            ...
            <input 
              ...
              className="w-full px-3 py-2 border border-[#D4AF37]/20 bg-slate-950 text-[#D4AF37] rounded-xl focus:outline-none focus:ring-1 focus:ring-[#D4AF37] font-semibold text-sm transition"
              step="0.01" 
          ...
          <div className="space-y-1.5">
            ...
            <input 
              type="number" 
              value={tax}
              onChange={(e) => setTax(e.target.value === '' ? '' : e.target.value)}
              className="w-full px-3 py-2 border border-[#D4AF37]/20 bg-slate-950 text-[#D4AF37] rounded-xl focus:outline-none focus:ring-1 focus:ring-[#D4AF37] font-semibold text-sm transition"
              step="0.1" 
              placeholder="np. 12"
            />
